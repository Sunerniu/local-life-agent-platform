...
public class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    private static final String DUMMY_PASSWORD = AccountPassword.encode("dummy-password-never-used-for-login");
    @Autowired
...
// 3. 检查过去1分钟内发送验证码的次数
    ...
    public Result login(LoginFormDTO loginForm, HttpSession session) {
        ...
        //2. 不符合格式则报错
        if (Cachecode==null || code == null || !code.equals(Cachecode))
        {
    ...
    @Override
    public Result loginWithPassword(String account, String password) {
        if (account == null || account.isBlank() || account.length() > 128
                || password == null || password.isEmpty() || password.length() > 128) {
            return Result.fail("账号或密码不正确");
        }
        String normalized = account.trim();
        String bucket = "login:password:attempts:" + org.springframework.util.DigestUtils
                .md5DigestAsHex(normalized.getBytes(java.nio.charset.StandardCharsets.UTF_8));
        Long attempts = stringRedisTemplate.opsForValue().increment(bucket);
        if (Long.valueOf(1).equals(attempts)) stringRedisTemplate.expire(bucket, 10, TimeUnit.MINUTES);
        if (attempts == null || attempts > 10) return Result.fail("尝试次数过多，请 10 分钟后重试");
        User user = query().eq("phone", normalized).one();
        String encoded = user != null && user.getPassword() != null && user.getPassword().startsWith("pbkdf2$")
                ? user.getPassword() : DUMMY_PASSWORD;
        boolean matched = AccountPassword.matches(encoded, password);
        if (user == null || encoded.equals(DUMMY_PASSWORD) || !matched) return Result.fail("账号或密码不正确");
        stringRedisTemplate.delete(bucket);
        String token = UUID.randomUUID().toString(true);
        HashMap<String, String> session = new HashMap<>();
        session.put("id", user.getId().toString());
        session.put("nickName", user.getNickName() == null ? "商家用户" : user.getNickName());
    ...
    public Result logout(String token) {
        if (token != null && !token.isBlank()) stringRedisTemplate.delete(LOGIN_USER_KEY + token);
        return Result.ok();
    }
